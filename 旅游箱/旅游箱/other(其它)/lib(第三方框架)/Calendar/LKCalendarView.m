...
-(void)startLoadingView
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(tempDateComponents)
        {
            ...
            self.currentDateComponents = [currentLKCalendar components:NSCalendarUnitYear|NSCalendarUnitMonth fromDate:[NSDate date]];
        }
    });
}
-(void)reloadViewDate
...
-(void)setCurrentDateComponents:(NSDateComponents *)com animation:(BOOL)animation
{
    ...
    if(com.month > 12 || com.month < 1)
    {
        ...
        com = [currentLKCalendar components:NSCalendarUnitYear|NSCalendarUnitMonth fromDate:date];
    }
    if(_currentDateComponents != nil)
    {
        long monthdiff = monthDiffWithDateComponents(_currentDateComponents,com);
        if(monthdiff==0)
            return;
        
        int contentOffsetX = self.frame.size.width;
        if(monthdiff > 0)
        {
...
{
    NSDateComponents* com = [currentLKCalendar components:NSCalendarUnitYear|NSCalendarUnitMonth|NSCalendarUnitDay fromDate:date];
    long monthdiff = monthDiffWithDateComponents(_currentDateComponents,com);
    ...
    long  index = [monthView.dates indexOfObject:date];
