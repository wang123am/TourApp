...
-(void)reloadMonthViewDate
{
    ...
    //获得第一天 是周几
    long firstWeekDay = [currentLKCalendar components:NSCalendarUnitWeekday fromDate:firstDay].weekday;
...
-(void)refreshMonthView
{
    ...
    {
        for (long i =0,size=_dayViews.count; i<size; i++) {
...
{
    long index = [self.dates indexOfObject:date];
